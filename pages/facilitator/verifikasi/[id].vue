...
const getDetail = async () => {
  try {
    ...
    if (response.code != 2000) navigateTo("/facilitator/verifikasi");
    ...
    if (jenisFasilitasi.value == "Reguler") url = "/facilitate/verifikator/lph";
...
const loadSOF = async () => {
  try {
    ...
    if (response.length) dataSOF.value = response;
    else useSnackbar().sendSnackbar("Ada Kesalahan asd", "error");
  } catch (error) {
    useSnackbar().sendSnackbar("Ada Kesalahan", "error");
  }
};
...
<template>
  ...
  <VRow class="d-flex justify-start align-center px-4">
    <h1 style="font-size: 32px">Detail Ajuan Fasilitasi</h1>
  </VRow>
  <VRow
