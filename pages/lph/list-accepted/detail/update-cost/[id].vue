...
const openModalEditDalamNegri = (item: any) => {
  ...
  const isManipulated = Object.values(item).some((value) =>
    value.toString().includes("Rp")
  );
  if (!isManipulated) {
  ...
  try {
    ...
    if (response?.code === 2000) {
      ...
      if (response.data.lph_admin) {
        response.data.biaya_indo.list.push({
          nama_pabrik: "Biaya Admin",
          subtotal: formatToIDR(response.data.lph_admin),
        });
      ...
      arrayAudit.push({
        keterangan: "Biaya Pemeriksaan LPH",
        subtotal: formatToIDR(total),
      });
      ...
      totalHargaAudit.value = arrayAudit.reduce(
        (total, item) => total + idrToNumber(item.subtotal),
        0
      );
    } else {
...
const getBiayaPesawat = async (id_pabrik: string) => {
  try {
    const response: any = await $api(
      "/reguler/lph/update-cost/detail-pesawat",
      {
        method: "get",
        params: { id, id_pabrik },
      }
    ...
    if (response?.code === 2000) biayaPabrik.value = formatToIDR(response.data);
    else useSnackbar().sendSnackbar("Ada Kesalahan", "error");
...
const onEdit = async () => {
  try {
    const body = {
      akomodasi_akhir:
        detailData.value?.akomodasi_akhir !== 0
          ? idrToNumber(detailData.value?.akomodasi_akhir)
          : 0,
      akomodasi_diskon: +detailData.value?.akomodasi_diskon,
      id_pabrik: detailData.value?.id_pabrik,
      tiket_pesawat_akhir:
        detailData.value?.tiket_pesawat_akhir !== 0
          ? idrToNumber(detailData.value?.tiket_pesawat_akhir)
          : 0,
      tiket_pesawat_diskon: +detailData.value?.tiket_pesawat_diskon,
      transport_akhir:
        detailData.value?.transport_akhir !== 0
          ? idrToNumber(detailData.value?.transport_akhir)
          : 0,
      transport_diskon: +detailData.value?.transport_diskon,
      uhpd_akhir:
        detailData.value?.uhpd_akhir !== 0
          ? idrToNumber(detailData.value?.uhpd_akhir)
          : 0,
      uhpd_diskon: +detailData.value?.uhpd_diskon,
      unit_cost_akhir:
        detailData.value?.unit_cost_akhir !== 0
          ? idrToNumber(detailData.value?.unit_cost_akhir)
          : 0,
...
const onDeletePesawat = async (el: any) => {
  try {
    const response: any = await $api(
      "/reguler/lph/update-cost/delete-biaya-pesawat",
      {
        method: "delete",
        body: {
          id,
  ...
  <div v-if="!loading">
    ...
    <VRow cols="8">
      <VCol>
        ...
        <VRow style="float: inline-end; margin-inline-end: 10px">
          <VBtn
            variant="flat"
            class="px-4"
            color="primary"
          ...
          <VCardText>
            ...
            >
              <template #body="{ items }">
                ...
                <tr v-for="(item, idx) in dataDomestic" :key="idx">
                  <td>{{ idx + 1 }}</td>
              ...
              <template #body="{ items }">
                ...
                <tr v-for="(item, idx) in dataAudit" :key="idx">
                  ...
                  <td class="d-flex justify-center align-center" width="150px">
                    {{
                      formatToIDR(
                        totalHargaAudit + data?.biaya_ln?.total_biaya
                      ) || 0
                    }}
                  </td>
                </tr>
    ...
    <VDialog v-model="addPesawat" max-width="840px" persistent>
      <VCard class="pa-4">
        ...
        <VCardActions class="pt-2 px-4">
          <VBtn variant="outlined" class="px-4 me-3" @click="addPesawat = false"
            >Batal</VBtn
          >
          <VBtn
            variant="flat"
    ...
    <VDialog v-model="addLn" max-width="840px" persistent>
      <VCard class="pa-4">
          ...
          <VRow>
            ...
            <VCol>
              ...
              <VTextField
                ...
                @update:model-value="
                  detailDataLn.total = detailDataLn.qty * detailDataLn.harga
                "
              />
            </VCol>
            <!-- <VCol>
        ...
        <VCardActions class="pt-2 px-4">
          <VBtn variant="outlined" class="px-4 me-3" @click="addLn = false"
            >Batal</VBtn
          >
          <VBtn
            variant="flat"
    ...
    <VDialog v-model="editLn" max-width="840px" persistent>
      <VCard class="pa-4">
        ...
        <VCardActions class="pt-2 px-4">
          <VBtn variant="outlined" class="px-4 me-3" @click="editLn = false"
            >Batal</VBtn
          >
          <VBtn
            variant="flat"
    ...
    <VDialog v-model="editDialog" max-width="840px" persistent>
      <VCard class="pa-4">
        <VCardTitle class="d-flex justify-space-between align-center">
          <div class="text-h3 font-weight-bold">Ubah Biaya Audit</div>
          <VIcon
            @click="
              () => {
                editDialog = false;
                  ...
                  (e) => {
                    ...
                    if (+e.target.value) {
                      const initialCost = idrToNumber(
                        detailData.unit_cost_awal
                      );
                      detailData.unit_cost_akhir =
                        initialCost - initialCost * (+e.target.value / 100);
                      if (detailData.unit_cost_akhir) {
                        detailData.unit_cost_akhir = formatToIDR(
                          detailData.unit_cost_akhir
                        );
                      }
                    } else {
                ...
                @input="
                  (e) => {
                    ...
                    if (+e.target.value) {
                      ...
                      detailData.uhpd_akhir =
                        initialCost - initialCost * (+e.target.value / 100);
                      if (detailData.uhpd_akhir) {
                        detailData.uhpd_akhir = formatToIDR(
                          detailData.uhpd_akhir
                        );
                      }
                    } else {
              ...
              <VTextField
                ...
                @input="
                  (e) => {
                    ...
                    if (+e.target.value) {
                      const initialCost = idrToNumber(
                        detailData.transport_awal
                      );
                      detailData.transport_akhir =
                        initialCost - initialCost * (+e.target.value / 100);
                      if (detailData.transport_akhir) {
                        detailData.transport_akhir = formatToIDR(
                          detailData.transport_akhir
                        );
                      }
                    } else {
              ...
              <VTextField
                ...
                @input="
                  (e) => {
                    ...
                    if (+e.target.value) {
                      const initialCost = idrToNumber(
                        detailData.akomodasi_awal
                      );
                      detailData.akomodasi_akhir =
                        initialCost - initialCost * (+e.target.value / 100);
                      if (detailData.akomodasi_akhir) {
                        detailData.akomodasi_akhir = formatToIDR(
                          detailData.akomodasi_akhir
                        );
                      }
                    } else {
              ...
              <VTextField
                ...
                @input="
                  (e) => {
                    ...
                    if (+e.target.value) {
                      const initialCost = idrToNumber(
                        detailData.tiket_pesawat_awal
                      );
                      detailData.tiket_pesawat_akhir =
                        initialCost - initialCost * (+e.target.value / 100);
                      if (detailData.tiket_pesawat_akhir) {
                        detailData.tiket_pesawat_akhir = formatToIDR(
                          detailData.tiket_pesawat_akhir
                        );
                      }
                    } else {
