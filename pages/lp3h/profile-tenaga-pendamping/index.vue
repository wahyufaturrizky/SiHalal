  ...
  try {
    ...
    if (response.code != 4000) {
      dataProfilePendamping.value.forEach(el => {
        if (el.label === 'NIK')
          el.value = response.data.pendamping.nik
        if (el.label === 'Nama')
          el.value = response.data.pendamping.nama
        if (el.label === 'Alamat')
          el.value = response.data.pendamping.alamat
        ...
        if (el.label === 'Email')
          el.value = response.data.pendamping.email
        if (el.label === 'Telp /HP')
          el.value = response.data.pendamping.no_hp
      ...
      dataBank.value.forEach(el => {
        if (el.label === 'Nama Bank')
          el.value = response.data.rekening.bank
      ...
      dataRegistrasi.value.forEach(el => {
        if (el.label === 'Status')
          el.value = response.data.pendamping.status
      ...
      documentLMS.value.forEach(el => {
        if (el.label === 'Pendalaman LMS')
          el.value = response.data.great_edu
      })
      ...
      dokumenPersyaratan.value.forEach(el => {
        ...
        if (el.label === 'KTP')
          el.value = response.data.pendamping.fotoktp
...
const handleSave = async () => {
  ...
  dataProfilePendamping.value.forEach(el => {
    ...
    if (el.label === 'Nama')
      body.nama = el.value
    if (el.label === 'Alamat')
      body.alamat = el.value
    if (el.label === 'Provinsi')
      body.provinsi = el.value
    if (el.label === 'Kota/Kab')
      body.kabupaten = el.value
    if (el.label === 'Kecamatan')
      body.kecamatan = el.value
    if (el.label === 'Kode Pos')
      body.kode_pos = el.value
    if (el.label === 'Email')
      body.email = el.value
    if (el.label === 'Telp /HP')
      body.no_hp = el.value
    if (el.label === 'Tempat Lahir')
      body.tempat_lahir = el.value

    if (el.label === 'Tanggal Lahir')
      body.tgl_lahir = convertToISOString(el.value)
    if (el.label === 'Pekerjaan')
      body.pekerjaan = el.value
    if (el.label === 'Pekerjaan_lain')
      body.pekerjaan_lain = el.value
    if (el.label === 'IDLembaga')
      body.IDLembaga = el.value
    if (el.label === 'ID pendamping')
      id_pendamping = el.value
  })

  dataPendidikan.value.forEach(el => {
    if (el.label === 'Pendidikan Terakhir')
      body.pendidikan = el.value
    if (el.label === 'Nama Universitas')
      body.universitas = el.value
  })

  dataBank.value.forEach(el => {
    if (el.label === 'Nama Bank')
      body.rekening.bank = el.value
    if (el.label === 'No. Rekening')
      body.rekening.no_rekening = el.value
    if (el.label === 'Nama Rekening')
      body.rekening.nama = el.value
    if (el.label === 'File Rekening')
      body.rekening.file_foto_rek = el.value
  })

  dataBank2.value.forEach(el => {
    if (el.label === 'NPWP')
      body.rekening.npwp = el.value
    if (el.label === 'Nama pada NPWP')
      body.rekening.nama_npwp = el.value
    if (el.label === 'File NPWP')
      body.rekening.file_foto_npwp = el.value
  })

  dokumenPersyaratan.value.forEach(el => {
    if (el.label === 'Ijazah')
      body.foto_ijazah = el.value
    else
      body.foto_ijazah = ''

    if (el.label === 'KTP')
      body.foto_ktp = el.value
    else
      body.foto_ktp = ''

    if (el.label === 'Sertifikat Pelatihan')
      body.fotosertifikat = el.value
  })
      ...
      <VCol cols="8">
        ...
        >
          <VExpansionPanel>
            ...
            <VExpansionPanelText>
              <VRow
                v-for="(item, index) in dataProfilePendamping"
                :key="index"
              >
                <VCol
                  v-if="
          ...
          <VExpansionPanel>
            ...
            <VExpansionPanelText>
              ...
              >
                ...
                <VCol
                  v-if="item.label === 'Pendidikan Terakhir'"
                  cols="12"
                >
                  <VSelect
                    v-model="item.value"
                ...
                <VCol
                  v-if="item.label === 'Nama Universitas'"
                  cols="12"
                >
                  <VTextField
                    v-model="item.value"
