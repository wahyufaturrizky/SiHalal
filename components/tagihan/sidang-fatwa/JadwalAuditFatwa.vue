  ...
  newData => {
    ...
    if (newData) {
      profil.value = [
        {
          id: 1,
          key: t('task-force.proses-sidang.detail.section-jadwal-audit.start-date'),
          value: getValueOrDash(newData?.sertifikat?.no_daftar),
        },
        {
          id: 2,
          key: t('task-force.proses-sidang.detail.section-jadwal-audit.end-date'),
  ...
  <VExpansionPanels v-model="paneSwitcher">
    <VExpansionPanel>
      <VExpansionPanelTitle><h2>
        {{ t('task-force.proses-sidang.detail.section-jadwal-audit.title') }}
