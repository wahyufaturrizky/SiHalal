<template>
  ...
  <VDialog v-model="dialogVisible" :max-width="dialogMaxWidth">
    <VCard class="pa-4">
      ...
      <VRow>
        <VCol>
          <VDataTable :headers="props.headers" :items="filteredItems">
            <template #item.check="{ item }">
              <VCheckbox
                v-model="checkedItems[item.no]"
              />
            </template>
          </VDataTable>
        </VCol>
      ...
      <VRow class="mt-3">
        <VCol class="d-flex justify-end">
          <VBtn @click="cancelSelection" color="primary" variant="outlined" class="mr-2">Cancel</VBtn>
          <VBtn @click="addSelection" color="primary">Add ({{checkedItemsCount}})</VBtn>
        </VCol>
      </VRow>
    </VCard>
  </VDialog>
