...
function removeUUID(fileName) {
  try {
    const updatedFileName = fileName.replace(
      /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}__/,
      ""
    );
...
const auditInternal = ref({
  label: [
    ...
    {
      title: "Unggahan Bukti Pemenuhan",
      key: "pemenuhanEvidence",
      nowrap: true,
    },
...
const uploadFile = async (file) => {
  ...
  const response = await $api(
    "/reguler/pelaku-usaha/tab-evaluasi/upload-file",
    {
      method: "post",
      query: { id },
      body: form,
...
const addTtd = async () => {
  try {
    if (actionOptions.value === "ADD") {
      ...
      const response = await $api(
        "/reguler/pelaku-usaha/tab-evaluasi/add-ttd",
        {
          method: "post",
          query: { id },
          body: {
      ...
      const response = await $api(
        "/reguler/pelaku-usaha/tab-evaluasi/update-ttd",
        {
          method: "put",
...
const getDokumenLainnya = async () => {
  try {
    const response = await $api(
      "/reguler/pelaku-usaha/tab-evaluasi/list-dokumen",
      {
        method: "get",
        params: { id },
      }
...
const handleAddOrEditDokumenLainya = async () => {
  try {
    if (actionOptions.value === "ADD") {
      ...
      if (responseUpload.code !== 2000) return;

      const response = await $api(
        "/reguler/pelaku-usaha/tab-evaluasi/add-dokumen",
        {
          method: "post",
          query: { id },
          body: {
      ...
      const response = await $api(
        "/reguler/pelaku-usaha/tab-evaluasi/update-dokumen",
        {
          method: "put",
...
const deleteDokumenLainnya = async (item) => {
  try {
    const response = await $api(
      "/reguler/pelaku-usaha/tab-evaluasi/delete-dokumen",
      {
        method: "delete",
...
const getDetail = async () => {
  try {
    ...
    if (response?.code === 2000) {
      ...
      auditInternal.value = {
        ...auditInternal.value,
        value: data.formulir_data_periksa_audit_internal?.map((i) => ({
...
const handleSubmitDokumen = async () => {
  if (
    titleDialog.value === "Tambah Dokumen" ||
    titleDialog.value === "Ubah Dokumen"
  )
    await handleAddOrEditDokumenLainya();

  if (
    titleDialog.value === "Tambah Tanda Tangan" ||
    titleDialog.value === "Ubah Tanda Tangan"
  )
    await addTtd();
};
...
const getListPenyelia = async () => {
  try {
    const response = await $api(
      "/reguler/pelaku-usaha/tab-evaluasi/list-penyelia",
      {
        method: "get",
        params: { id },
      }
    ...
    if (response.code === 2000) {
      penyeliaOptions.value = response.data?.map((i) => ({
        title: i.nama,
        value: i.nama,
      }));
    }
    ...
    >
      <template #content>
        <div v-if="titleDialog === 'Tambah Dokumen'">
          <label class="label-pengajuan"> Nama Dokumen </label>
          <VTextField
        ...
        <div v-if="titleDialog === 'Tambah Tanda Tangan'">
          ...
          <label> Nama Penyelia Halal </label>
          <VSelect
        ...
        <div v-if="titleDialog === 'Ubah Dokumen'">
          <p class="label-pengajuan">Nama Dokumen</p>
          <VTextField
            v-model="formDokumenLainnya.nama_dokumen"
            class="-mt-10"
            placeholder="isi nama dokumen"
        ...
        <div class="bgContent">
          ...
          >
            ...
            <ol>
              ...
              <li class="subText">Format dokumen berupa PDF</li>
            </ol>
          </div>
        </div>
      </template>
